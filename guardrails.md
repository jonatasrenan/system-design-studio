...
No session can be marked `done` without passing this checklist (via `/review` or the closing phase of `/design`). Each item gets one of **five verdicts**: **PASS** (addressed, with justification), **FAIL** (real gap — needs a decision or a conscious out-of-scope note), **N/A** (does not apply, with a reason), `[premise-to-validate]` (can't be judged yet), or **ACCEPTED RISK** (a FAIL the user knowingly decided to carry).

**Third state — `[premise-to-validate]`**: an item nobody can judge yet (the design doesn't say enough either way). It counts in `guardrails.premises`, never in `fail` or `pass` — marking it PASS or FAIL anyway would either fake coverage or invent a decision that reaches the next reader looking closed. It only leaves this state once someone actually validates it (it becomes PASS or FAIL); it never resolves itself by turning into a risk, and it never resolves itself by the clock running out.

**Fourth state — ACCEPTED RISK**: a FAIL the user chose to carry rather than fix. It counts in `guardrails.accepted_risks`, not in `fail`. Two preconditions are mandatory before an item can carry this verdict: an entry in the scorecard's `risks`, **and** a recorded decision in `40-tradeoffs.md` (options, choice, what's given up, a "30s defense"). Without both, the item stays FAIL — a risk nobody wrote down isn't accepted, it's just unaddressed. ACCEPTED RISK never becomes PASS: accepting a risk doesn't mean the design addresses it.
...
The gate: `pass + fail + na + premises + accepted_risks` must equal the total number of items in this file — the lint enforces the closed sum, so an item nobody has judged yet can't silently disappear from the count. `status: "done"` is blocked only while `fail > 0`; accepted risks and open premises never block it.
